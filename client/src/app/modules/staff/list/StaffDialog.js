...
function StaffDialog(props) {
  ...
  const staffDialog = useSelector(({ staffApp }) => staffApp.staff.staffDialog);
  ...
  const initDialog = useCallback(() => {
     ...
     * Dialog type: 'edit'
     */
    if (staffDialog.type === 'edit' && staffDialog.data) {
      setForm({ ...staffDialog.data });
    }

    /**
     * Dialog type: 'new'
     */
    if (staffDialog.type === 'new') {
      setForm({
        ...defaultFormState,
        ...staffDialog.data,
  ...
  }, [staffDialog.data, staffDialog.type, setForm]);

  useEffect(() => {
    /**
     * After Dialog Open
     */
    if (staffDialog.props.open) {
      initDialog();
    }
  }, [staffDialog.props.open, initDialog]);

  function closeStaffDialog() {
    staffDialog.type === 'edit'
      ? dispatch(Actions.closeEditStaffDialog())
      : dispatch(Actions.closeNewStaffDialog());
  }
  ...
  return (
    ...
    >
      <AppBar position='static' elevation={1}>
        <Toolbar className='flex w-full'>
          <Typography variant='subtitle1' color='inherit'>
            {staffDialog.type === 'new' ? 'New Staff' : 'Edit Staff'}
          </Typography>
        </Toolbar>
      </AppBar>
      ...
      {staffDialog.type === 'new' ? (
        <DialogActions className='justify-between pl-8 sm:pl-16'>
          <Button
            variant='contained'
            color='primary'
            onClick={() => {
              dispatch(Actions.addStaff(form));
              closeStaffDialog();
            }}
        ...
        <DialogActions className='justify-between pl-8 sm:pl-16'>
          <Button
            ...
            color='primary'
            onClick={() => {
              dispatch(Actions.updateStaff(form));
              closeStaffDialog();
            }}
            disabled={!canBeSubmitted()}
          >
          ...
          <IconButton
            ...
            onClick={() => {
              dispatch(Actions.removeStaff(form.id));
              closeStaffDialog();
            }}
          >
            <Icon>delete</Icon>
