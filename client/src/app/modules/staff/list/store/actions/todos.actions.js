...
export function updateStaffList() {
...
export function toggleCompleted(todo) {
  ...
  return dispatch =>
    Promise.all([dispatch({ type: TOGGLE_COMPLETED })]).then(() =>
      dispatch(updateStaff(newStaffList))
    );
}
...
export function updateStaff(todo) {
  ...
  return dispatch =>
    request.then(response => {
      ...
      ]).then(() => dispatch(updateStaffList()));
    });
}

export function openNewStaffDialog() {
  return {
    type: OPEN_NEW_TODO_DIALOG
  };
}

export function closeNewStaffDialog() {
  return {
    type: CLOSE_NEW_TODO_DIALOG
  };
}

export function openEditStaffDialog(data) {
  return {
...
export function closeEditStaffDialog() {
  return {
    type: CLOSE_EDIT_TODO_DIALOG
  };
}

export function addStaff(todo) {
...
export function removeStaff(todoId) {
