...
function FirebasePanel() {
  ...
  // internal component state constants
  ...
  // todo: if they're already signed in, use this panel
  // to allow them to sign out
  return (
    <Fragment>
      <Formsy
        onValidSubmit={model => authenticate("password", model)}
        onValid={() => setComponentState(VALID)}
        onInvalid={() => setComponentState(INVALID)}
        ref={formRef}
